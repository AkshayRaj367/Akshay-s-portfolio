...
function createTextTexture(text, canvas) {
...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    // Debug: Clear canvas with a background color
    ctx.fillStyle = '#000000';
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // Create text textures instead of loading images
    console.log('Creating textures for items:', this.items);
    this.items.forEach((item, i) => {
      const textCanvas = document.createElement('canvas');
      const skillName = item.title || item.name || 'Skill';
      console.log(`Creating texture for skill ${i}: ${skillName}`);
      createTextTexture(skillName, textCanvas);
      
      const x = (i % this.atlasSize) * cellSize;
      const y = Math.floor(i / this.atlasSize) * cellSize;
      ctx.drawImage(textCanvas, x, y, cellSize, cellSize);
    });
